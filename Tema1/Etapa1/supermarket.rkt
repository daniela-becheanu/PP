...
(define (time-counter index time)
  (make-counter index time null))
...
(define (min-tt counters)
  (min-elem counters (car counters))
)

(define (min-elem counters C)
  (cond ((null? counters) (cons (counter-index C) (counter-tt C)))
        ((> (counter-tt C) (counter-tt (car counters))) (min-elem (cdr counters) (car counters)))
        (else (min-elem (cdr counters) C))))
  ...
  (if (null? requests)
      ...
      (match (car requests)
        [(list 'delay index minutes)
         (cond ((= 1 index) (serve (cdr requests) (tt+ C1 minutes) C2 C3 C4))
               ((= 2 index) (serve (cdr requests) C1 (tt+ C2 minutes) C3 C4))
               ((= 3 index) (serve (cdr requests) C1 C2 (tt+ C3 minutes) C4))
               ((= 4 index) (serve (cdr requests) C1 C2 C3 (tt+ C4 minutes))))]
        
        [(list name n-items)
         (if (<= n-items ITEMS)
             (cond ((= (car (min-tt (list C1 C2 C3 C4))) 1) (serve (cdr requests) (add-to-counter C1 name n-items) C2 C3 C4))
                   ...
                   ((= (car (min-tt (list C1 C2 C3 C4))) 4) (serve (cdr requests) C1 C2 C3 (add-to-counter C4 name n-items))))
             (cond ((= (car (min-tt (list C2 C3 C4))) 2) (serve (cdr requests) C1 (add-to-counter C2 name n-items) C3 C4))
                   ((= (car (min-tt (list C2 C3 C4))) 3) (serve (cdr requests) C1 C2 (add-to-counter C3 name n-items) C4))
                   ((= (car (min-tt (list C2 C3 C4))) 4) (serve (cdr requests) C1 C2 C3 (add-to-counter C4 name n-items)))))])))
