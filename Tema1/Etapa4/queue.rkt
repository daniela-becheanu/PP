...
(define (enqueue x q)
  (if (= (queue-size-l q) (queue-size-r q)) ; trebuie sa fac rotatie
      (match q
        [(queue left right size-l size-r)
         (struct-copy queue q
                      [left (rotate left (cons x right) empty-stream)]
      ...
      (match q
        [(queue left right size-l size-r)
         (struct-copy queue q [left (rotate (stream-rest left) right empty-stream)]
                      [right null]
                      [size-l (sub1 (+ size-r size-l))]
                      [size-r 0])])
      (match q
        [(queue left right size-l size-r)
         (struct-copy queue q [left (stream-rest left)]
                      [size-l (sub1 size-l)])])))
