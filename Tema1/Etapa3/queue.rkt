...
(define (enqueue x q)
  (match q
    [(queue left right size-l size-r)
     (struct-copy queue q [right (cons x right)]
                  [size-r (add1 size-r)])]))

(define (move left right) ;FA O NERECURSIV
  (if (null? right)
      left
      (move (cons (car right) left) (cdr right))))
...
(define (dequeue q)
  (if (null? (queue-left q))
      (match q
      [(queue left right size-l size-r)
     (struct-copy queue q [left (cdr (move left right))]
                  [right null]
                  [size-l (sub1 size-r)]
                  [size-r 0])])
      (match q
      [(queue left right size-l size-r)
     (struct-copy queue q [left (cdr left)]
                  [size-l (sub1 size-l)])])
      )
  )
